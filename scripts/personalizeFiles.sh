...
sed "s|EpochMod.com (0.5|1.68) ID02 YourHost|${SERVER_NAME}|" ${FORCE_INSTALL_DIR}/sc/server.cfg
sed 's|password         	= "";|password         	= "${SERVER_PASSWRD}";|' ${FORCE_INSTALL_DIR}/sc/server.cfg
sed 's|CHANGE_THIS_PASSWORD!|"${ADMIN_PASSWRD}"|' ${FORCE_INSTALL_DIR}/sc/server.cfg
sed "s|adminMenu_Owner[] = {};|adminMenu_Owner[] = {${ADMIN_ARRAY}};|" ${FORCE_INSTALL_DIR}/@epochhive/epochah.hpp
